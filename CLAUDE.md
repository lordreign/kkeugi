...
- **분석**: Mixpanel free
- **모니터링**: Sentry free tier (Flutter 만) · 백엔드 unhandled 에러 → Telegram 운영자 알림 (`TELEGRAM_OPS_CHAT_ID`, fingerprint 5분 cooldown)
...
| 2026-06-08 | **백엔드 Sentry → Telegram OPS 에러 알림으로 전환**. (1) 신규 `app/observability/error_notifier.py` — `make_fingerprint(path+exc_class+msg)` + `should_notify()` cooldown(기본 300s) + `format_message()` (4096자 안전, 마지막 5프레임 + skipped_since) + `notify_exception()` fire-and-forget(절대 raise X). (2) `main.py` 전역 `@exception_handler(Exception)` 으로 5xx 자동 캐치 → request.state.user 컨텍스트 첨부. (3) dev/test 환경 `/v1/_dev/boom` 검증 endpoint. (4) sentry-sdk 의존성·`sentry_sdk.init` 블록·`sentry_dsn` config 필드 완전 제거. (5) config 신규 `TELEGRAM_OPS_CHAT_ID` + `ERROR_NOTIFIER_COOLDOWN_SEC`. (6) 단위 테스트 12건(fingerprint·cooldown 결정론·format trace truncate·skipped count) → backend **76→88 green**. (7) 문서 정리: README·CLAUDE·PRD §7 모니터링·ARCHITECTURE 다이어그램·REMAINING-WORK·REMAINING-WORK-email·W2-prestart-P0·.env.example — Sentry 백엔드 항목을 "Telegram OPS 알림"으로 교체. 프론트 Sentry 는 유지. | 사용자: "Sentry 안 쓰고 Telegram 으로 에러 받겠다". 이미 검증된 봇 인프라 재사용 → 외부 의존 -1, 모바일 알림 즉시(Sentry 앱 X), 운영 cognitive load 감소. fingerprint cooldown 으로 spam 통제. |
| 2026-06-05 | **V1_EXECUTION_PLAN v2 + 출시 전 7개 작업 일괄 반영**. 1)Threshold 무료 1개+유료 2개~(백엔드 require_paid 제거→첫 1개 free, 이후 402 / 프론트 paywall 트리거 조건 변경+"무료는 1개까지·Pro" 카피). 2)손실 누적 paywall(신규 `GET /v1/usage/stats/month-loss` 월초 ~ 현재 minutes·loss_won·hourly_value 응답 + paywall 진입 시 클레이 banner 표시·홈 노출 X+`paywall_viewed` 이벤트). 3)포지셔닝 "디지털 디톡스"→"시간빚 추적"(PRD §1·README). 4)KPI 재정의(D30 12%/공유 5%/paid 4-7% + Mixpanel 4 view 셋업 가이드). 5)PRD §14 **Employment Risk** 신규 섹션(6관리축·발각 risk 정량화·모니터링 cadence). 6)PRD §15 V2 우선순위 재정렬(V1.5=Streak·공유강화·AI코치, V2=사업자등록·Toss·카톡, iOS는 V2후반/V3로 격하). 7)Feature flag 인프라(`feature_flags.py` 결정론적 bucket + `GET /v1/feature_flags` + 프론트 `FeatureFlags`/`featureFlagsProvider`). 신규문서: `V1_EXECUTION_PLAN.md`·`V1_STORE_LISTING.md`·`V1_MIXPANEL_SETUP.md`·`kkeugi_business_review.md`·`REMAINING-WORK-email.md`+PDF. 검증: backend 69→76 green(+7), frontend 25 green, debug APK 빌드. | 매출환산 가설 검증(시간 vs 돈) 1가설 압축 + Pivot Trigger(M1 paid<2% AND 정성 4/5)로 V1.5 AI 코치 path 결정. |
| 2026-06-02 | **applicationId 변경**: `kr.kkeugi.kkeugi` → **`kr.pjshi.kkeugi`** (브랜드 네임스페이스, Play Console 업로드 **전** 마지막 변경 기회). `build.gradle.kts` namespace+applicationId / Kotlin 경로 `kr/kkeugi/kkeugi/` → `kr/pjshi/kkeugi/` (`MainActivity.kt` package) / MethodChannel `kr.kkeugi/usage` → `kr.pjshi/usage` (브랜드 일관성) / 백엔드 `config.google_play_package_name` 기본값 / 문서 4건(REMAINING-WORK·W2-prestart·W5-prestart·index). `flutter clean`+pub get+analyze 0+25 green+`aapt2 dump packagename`=**kr.pjshi.kkeugi** 확인. 업로드 후엔 영구 변경 불가. | 패키지명은 Play Store 영구 identity. 업로드 전이라 무비용 변경. README historical 로그는 유지. |
